...
def FULL_3DES_CBC_MAC(key, icv, msg):

    k = binascii.unhexlify(key)
    iv = binascii.unhexlify(icv)
    
    # mandatory padding 80 00 ...
    msg1 = PADDING80(msg, DES_BLOCK_SIZE)
    data = binascii.unhexlify(msg1)

    tdes_key = triple_des(k, ECB, None, None, PAD_NORMAL)

    len_3des_data = len(data)

    c = iv
    off = 0
    while len_3des_data > 0:
        #print 'm=' + binascii.hexlify(data[off:off+8])
        m = XOR_BYTE(c, data[off:off+8])
        #print 'xor(m)=' + binascii.hexlify(m)
